<template>
  <ul
    v-if="context.active"
    ref="contextmenu"
    class="absolute z-30 select-none rounded border border-neutral-300 bg-neutral-50 text-xs text-gray-700 shadow dark:border-gray-600 dark:bg-gray-800 dark:text-gray-200"
    :style="context.positions"
  >
    <li
      v-for="item in context.items"
      :key="item.title"
      class="cursor-pointer px-2 py-1.5 hover:bg-neutral-200 dark:hover:bg-gray-700"
      @click="run(item)"
    >
      <span class="px-1" />
...
const menuItems = {
  newfolder: {
    title: () => t("New Folder"),
    action: () => {
      emitter.emit("vf-modal-show", { type: "NewFolder" })
    },
  },
  delete: {
    title: () => t("Delete"),
