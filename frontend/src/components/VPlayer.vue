...
<template>
  ...
  <div v-show="status === TStatus.SUCCESS" class="flex h-full min-h-[400px] w-full flex-col justify-center">
    <video ref="videoRef" id="video" crossorigin="anonymous" class="max-h-[600px] w-full" />
  </div>
  <div class="space-between absolute bottom-0 left-0 w-full border-t-[1px] border-zinc-100 bg-white py-1 pl-5 pr-1">
            ...
            <div class="max-w-[300px]">
              ...
              <div class="mt-1 flex justify-between space-x-3">
                ...
                <v-camera-control-item layout="right" icon="i-lucide-arrow-right">
                  <a-slider style="width: 100px" v-model:value="cameraControl.panRight" :min="0" :max="1" :step="0.1" />
                </v-camera-control-item>
              </div>
              ...
              <div class="flex justify-between space-x-3">
                ...
                <v-camera-control-item layout="right" icon="i-lucide-zoom-out">
                  <a-slider style="width: 100px" v-model:value="cameraControl.zoomOut" :min="0" :max="1" :step="0.1" />
                </v-camera-control-item>
              </div>
            </div>
          </template>
          <a-button class="flex items-center justify-center text-zinc-600">
            <span class="i-lucide-switch-camera text-zinc-600 h-4 w-4"></span>
          </a-button>
        </a-popover>
      </div>
    </div>
