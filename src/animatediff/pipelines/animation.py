...
class PromptEncoder:
    ...
    def get_current_prompt_embeds_from_text(self, center_frame=None, video_length: int = 0):
        ...
        for prompt_map, prompt_embeds_map in zip(self.prompt_map_list, self.prompt_embeds_region_list):
            embs = self._get_current_prompt_embeds_from_text(prompt_map, prompt_embeds_map, center_frame, video_length)
            outputs += (embs,)
    ...
    def get_current_prompt_embeds_from_image(self, center_frame=None, video_length: int = 0):
        ...
        for prompt_map, prompt_embeds_map in zip(self.ip_im_map_list, self.im_prompt_embeds_region_list):
            embs = self._get_current_prompt_embeds_from_image(prompt_map, prompt_embeds_map, center_frame, video_length)
            outputs += (embs,)
...
class AnimationPipeline(DiffusionPipeline, TextualInversionLoaderMixin):
    ...
    def prepare_controlnet_ref_only_without_motion(
        ...
        uc_mask = (
            torch.Tensor(
                [1] * batch_size * num_images_per_prompt + [0] * batch_size * num_images_per_prompt * (region_num - 1)
            )
    ...
    def __call__(
            ...
            gradual_latent_map: TGradualLatentHiresFixMap | None = None,
            **kwargs,
    ):
        global C_REF_MODE
        ...
        if gradual_latent:
            ...
            logger.warning(f"gradual latent {reverse_steps=} {noise_add_count=} {gradual_latent_map.scale.keys()=} ", )
                            ...
                            if False:
                                ...
                                if gradual_latent:
                                    ...
                                    cont_var_img = torch.nn.functional.interpolate(
                                        cont_var_img.float(), size=(cur_lat_height * 8, cur_lat_width * 8),
    ...
    def progress_bar(self, iterable=None, total=None):
        ...
        if iterable is not None:
            return tqdm(iterable, **self._progress_bar_config, desc="Step 07/08 Animating")
        elif total is not None:
            return tqdm(total=total, **self._progress_bar_config, desc="Step 07/08 Animating")
        else:
