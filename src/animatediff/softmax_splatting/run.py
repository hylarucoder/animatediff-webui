...
for strOption, strArgument in getopt.getopt(
    ...
    if strOption == "--video2" and strArgument != "":
        arguments_strVideo2 = strArgument  # path to a video
...
def backwarp(tenIn, tenFlow):
    if str(tenFlow.shape) not in backwarp_tenGrid:
        tenHor = (
            torch.linspace(start=-1.0, end=1.0, steps=tenFlow.shape[3], dtype=tenFlow.dtype, device=tenFlow.device)
            .view(1, 1, 1, -1)
            .repeat(1, 1, tenFlow.shape[2], 1)
        )
        tenVer = (
            torch.linspace(start=-1.0, end=1.0, steps=tenFlow.shape[2], dtype=tenFlow.dtype, device=tenFlow.device)
            .view(1, 1, -1, 1)
    ...
    def forward(self, tenOne, tenTwo):
        ...
        return {
            "tenForward": torch.nn.functional.interpolate(
                input=objForward["tenForward"], size=(intHeight, intWidth), mode="bilinear", align_corners=False
            )
            * (float(intWidth) / float(objForward["tenForward"].shape[3])),
            "tenBackward": torch.nn.functional.interpolate(
                input=objBackward["tenForward"], size=(intHeight, intWidth), mode="bilinear", align_corners=False
            )
...
class Synthesis(torch.nn.Module):
    def __init__(self):
        ...
        class Softmetric(torch.nn.Module):
            def __init__(self):
                ...
                self.netInput = torch.nn.Conv2d(
                    in_channels=3, out_channels=12, kernel_size=3, stride=1, padding=1, bias=False
                )
                self.netError = torch.nn.Conv2d(
                    in_channels=1, out_channels=4, kernel_size=3, stride=1, padding=1, bias=False
                )
                    ...
                    [
                        ...
                        self.netError(
                            torch.nn.functional.l1_loss(
                                input=tenEncone[0], target=backwarp(tenEnctwo[0], tenFlow), reduction="none"
                            ).mean([1], True)
                        ),
                    ],
                    1,
        ...
        class Warp(torch.nn.Module):
            ...
            def forward(self, tenEncone, tenEnctwo, tenMetricone, tenMetrictwo, tenForward, tenBackward):
                ...
                for intLevel in range(3):
                    if intLevel != 0:
                        tenMetricone = torch.nn.functional.interpolate(
                            input=tenMetricone,
                            size=(tenEncone[intLevel].shape[2], tenEncone[intLevel].shape[3]),
                            mode="bilinear",
                            align_corners=False,
                        )
                        tenMetrictwo = torch.nn.functional.interpolate(
                            input=tenMetrictwo,
                            size=(tenEnctwo[intLevel].shape[2], tenEnctwo[intLevel].shape[3]),
                            mode="bilinear",
                            align_corners=False,
                        )
                    ...
                    tenOutput.append(
                        [self.netOne, self.netTwo, self.netThr][intLevel](
                            torch.cat(
                                [
                                    softsplat.softsplat(
                                    ...
                                    softsplat.softsplat(
                                        tenIn=torch.cat([tenEnctwo[intLevel], tenMetrictwo], 1),
                                        tenFlow=tenBackward,
        ...
        for intRow, intFeatures in [(0, 32), (1, 64), (2, 96)]:
            ...
            self.add_module(
                str(intRow) + "x3" + " - " + str(intRow) + "x4",
                Basic("relu-conv-relu-conv", [intFeatures, intFeatures, intFeatures], True),
            )
            self.add_module(
                str(intRow) + "x4" + " - " + str(intRow) + "x5",
                Basic("relu-conv-relu-conv", [intFeatures, intFeatures, intFeatures], True),
            )
        # end
        ...
        self.load_state_dict(
            {
                ...
                for strKey, tenWeight in torch.hub.load_state_dict_from_url(
                    url="http://content.sniklaus.com/softsplat/network-" + arguments_strModel + ".pytorch",
                    file_name="softsplat-" + arguments_strModel,
                ).items()
            }
        )
    ...
    def forward(self, tenOne, tenTwo, fltTimes):
        with torch.set_grad_enabled(False):
            ...
            tenStd = (
                sum(
                    [
                        tenIn.std([1, 2, 3], False, True).square() + (tenMean - tenIn.mean([1, 2, 3], True)).square()
        ...
        tenImages = [
            self.netSynthesis(tenOne, tenTwo, objFlow["tenForward"], objFlow["tenBackward"], fltTime)
            for fltTime in fltTimes
        ]
...
def estimate(tenOne, tenTwo, fltTimes):
    ...
    return [
        ...
        for tenImage in netNetwork(tenPreprocessedOne, tenPreprocessedTwo, fltTimes)
    ]
    ...
    def forward(self, tenOne, tenTwo, guideFrameList):
        ...
        with torch.set_grad_enabled(False):
            ...
            gtenStd = (
                sum(
                    [
                        tenIn.std([1, 2, 3], False, True).square() + (gtenMean - tenIn.mean([1, 2, 3], True)).square()
                        for tenIn in gtenStats
                    ]
                )
                / len(gtenStats)
            ).sqrt()
...
def estimate2(img1: PIL.Image, img2: PIL.Image, guideFrames, model_file_path):
    ...
    def forTensor(im):
        return torch.FloatTensor(
            numpy.ascontiguousarray(
                numpy.array(im)[:, :, ::-1].transpose(2, 0, 1).astype(numpy.float32) * (1.0 / 255.0)
            )
        )
    ...
    tenGuideFrames = [
        torch.nn.functional.pad(input=ten, pad=[0, intPadr, 0, intPadb], mode="replicate") for ten in tenGuideFrames
    ]

    result = [
        tenImage[0, :, :intHeight, :intWidth].cpu()
        for tenImage in netNetwork(tenPreprocessedOne, tenPreprocessedTwo, tenGuideFrames)
    ]
    result = [
        PIL.Image.fromarray((r.clip(0.0, 1.0).numpy().transpose(1, 2, 0)[:, :, ::-1] * 255.0).astype(numpy.uint8))
        for r in result
    ]
