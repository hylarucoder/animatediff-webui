    ...
    if hasattr(pipeline, "controlnet"):
        ...
        if hasattr(pipeline.controlnet, "nets"):
            for i in range(len(pipeline.controlnet.nets)):
                pipeline.controlnet.nets[i] = pipeline.controlnet.nets[i].to(
                    device=device, dtype=vae_dtype, memory_format=model_memory_format
                )
        else:
            if pipeline.controlnet:
                pipeline.controlnet = pipeline.controlnet.to(
                    device=device, dtype=vae_dtype, memory_format=model_memory_format
                )
    ...
    if hasattr(pipeline, "controlnet_map"):
        if pipeline.controlnet_map:
            for c in pipeline.controlnet_map:
                ...
                pipeline.controlnet_map[c] = pipeline.controlnet_map[c].to(
                    dtype=unet_dtype, memory_format=model_memory_format
                )
